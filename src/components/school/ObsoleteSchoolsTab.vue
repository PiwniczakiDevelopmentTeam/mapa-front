...
<template>
  <div class="space-y-4">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
      ...
      <div v-if="totalCount > 0" class="flex flex-wrap items-center gap-3 text-sm text-gray-500">
        <button
        ...
        <span class="hidden sm:inline">Wierszy:</span>
        <select
          :value="pageSize"
    ...
    <div class="bg-white rounded-lg border border-gray-200 overflow-hidden">
      ...
      <div class="sm:hidden">
        ...
        <div v-else-if="schools.length === 0" class="px-4 py-12 text-center text-gray-400 text-sm">
        ...
        <div v-else class="divide-y divide-gray-100">
          <div
            ...
            :class="['p-4 transition-opacity', deletedIds.has(school.numerRspo) ? 'opacity-40' : '']"
          >
            <div class="flex items-start justify-between gap-2 mb-3">
              ...
              <span v-if="deletedIds.has(school.numerRspo)" class="shrink-0 text-xs text-emerald-600 font-medium">Usunięto</span>
              <span v-else-if="deleteErrorIds.has(school.numerRspo)" class="shrink-0 text-xs text-red-500">Błąd</span>
            </div>
            <div v-if="!deletedIds.has(school.numerRspo)" class="flex gap-2">
              <button
                @click="deleteOne(school.numerRspo)"
                :disabled="deletingIds.has(school.numerRspo)"
                class="flex-1 inline-flex items-center justify-center gap-1.5 px-3 py-2 rounded-md text-xs font-medium text-white bg-red-600 hover:bg-red-700 disabled:bg-gray-400 disabled:cursor-not-allowed transition-colors"
              >
                <svg v-if="deletingIds.has(school.numerRspo)" class="animate-spin w-3.5 h-3.5" fill="none" viewBox="0 0 24 24">
                  <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4" />
                  <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" />
                </svg>
                <svg v-else class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
